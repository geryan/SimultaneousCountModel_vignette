...
## Packages
```{r packages, result='hide', warning = FALSE, message = FALSE}
library(dplyr)
library(rjags)
library(R2jags)
library(jagstools)
...
```{r jags.fit, results = 'hide', warning = FALSE, message = FALSE}
# jags.fit <- jags(
#   data = jags.data,
#   inits = jags.inits,
...
```{r jags.fit parallel, results = 'hide', warning = FALSE, message = FALSE}
jags.fit <- do.call(
  what = jags.parallel,
  args = list(
    data = jags.data,
